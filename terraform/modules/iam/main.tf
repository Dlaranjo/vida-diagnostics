# IAM Module - Creates roles and policies for Lambda and Step Functions
    ...
    Statement = [
      {
        ...
        # Use wildcard to avoid circular dependency
...
# Policy for Step Functions to write CloudWatch metrics
resource "aws_iam_role_policy" "step_functions_cloudwatch" {
  name = "${var.project_name}-step-functions-cloudwatch"
  role = aws_iam_role.step_functions_execution.id
...
# Optional: VPC configuration for Lambda (if enabled)
